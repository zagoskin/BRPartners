...
internal sealed class OutboxDomainEventPublisher : BackgroundService
{
    ...
    {
        _logger = logger;
        _period = TimeSpan.FromSeconds(configuration.GetValue<int>("ServiceSecondIntervals:OutboxDomainEventPublisher"));
    ...
    private async Task PerformScopedPublishAndSaveResultsAsync(OutboxDomainEvent outboxDomainEvent, IDomainEvent domainEvent, CancellationToken stoppingToken)
    {
        ...
        var dbContext = eventScope.ServiceProvider.GetRequiredService<VendorManagementDbContext>();
        ...
        {
            dbContext.OutboxDomainEvents.Remove(outboxDomainEvent);
        }
